...
def record_note():
    """Record speech until user presses Enter, then transcribe."""
    recognizer = sr.Recognizer()
    mic = sr.Microphone()

    print("Recording note... Speak now.")
    print(">>> Press Enter when finished.")

    with mic as source:
        recognizer.adjust_for_ambient_noise(source, duration=1)
        try:
            # Listen until Enter is pressed
            print("(Listening...)")
    ...
    try:
        ...
        print(f"Captured Note: {note_text}")
        return note_text
...
def main():
    ...
    while True:
        ...
        if not emotion:
            emotion = input("Could not auto-detect. Enter emotion (Sad/Happy/Angry/Surprised): ").strip().capitalize()

        # Note via speech-to-text
        note = record_note()
        if not note:
            note = input("Could not capture speech. Enter note manually: ").strip()

        add_entry(user_id, mood, emotion, note)
        print(f"Entry saved with emotion: {emotion}, note: {note}\n")
